...
class UserContract extends Contract {
     ...
     * @param idNo - id No of the User
	 * @returns The transaction will return the User object which got added to blockchain.
	 */
	async requestNewUser(ctx, name, email,phoneNo, idNo) {
		
		if('usersMSP'!=ctx.clientIdentity.mspId){
			throw new Error('You are not authorized to perform this operation');
		}
		
		const userKey = ctx.stub.createCompositeKey('org.property-registration-network.regnet.user', [name,idNo]);	// Create a new composite key for the new User account
		let dataBuffer = await ctx.stub.getState(userKey).catch(err => console.log(err));
		if (dataBuffer.toString()) {
			throw new Error('Invalid User Details. An user with this name & idNo already exists.');
		} else {
	 ...
	 * @param name - Name of the User
     * @param idNo - Aadhaar No of the User
	 ...
	 */
	async rechargeUserAccount(ctx, name, idNo, bankTransactionId) {

		if('usersMSP'!=ctx.clientIdentity.mspId){
			throw new Error('You are not authorized to perform this operation');
		}

		const userKey = ctx.stub.createCompositeKey('org.property-registration-network.regnet.user', [name,idNo]);
		let dataBuffer = await ctx.stub.getState(userKey).catch(err => console.log(err));
		if (!dataBuffer.toString()) {
			throw new Error('Invalid User Details. No user exists with provided name & idNo combination.');
		}
	 ...
	 */
	async propertyRegistrationRequest(ctx,propertyId,price,status,name,idNo){
	 ...
	 * @returns The transaction will return the current state of any property.
	 */
	async updateProperty(ctx,propertyId,status,name,idNo){
     ...
     * @param idNo - Aadhaar No of the Buyer
	 * @returns The transaction will return the current state of any property.
	 */
	async purchaseProperty(ctx,propertyId,name,idNo){
